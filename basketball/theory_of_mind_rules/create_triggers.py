...
df = pd.read_csv('prototype\data\dataset_results.csv')
df_effects = pd.read_csv('prototype\data\\basketball_effects_nn.csv')
...
df.drop(['scenario','time'],axis=1,inplace=True)

chars = set(df.acting_char.value_counts().keys())
results_all = []

for char in chars:
    df3 = df.copy()
    df3 = df3[df3['acting_char'] == char]
    print(char)
    #print(df3)
...
df_all.to_csv('prototype\\data\\triggers.csv',index=False)
df_all = pd.read_csv('prototype\\data\\triggers.csv')
...
for index, row in df_all.iterrows():
    ...
    for action in row.results:
        i+=1
        text = 'action '
        text += 'predictaction_' + str(i) +  "_" + row.acting_char + "_" + action_name   +"(char : character){ \n"
        text += "precondition: \n"
        text += "later(world) & \n"
        ...
        text += effect + ' \n'
        text += "}; \n"
        actions_dict[text] = row.acting_char
        print(text)


df_actions = pd.DataFrame(data=actions_dict.items(),columns=['actions','char'])
df_actions.to_csv('prototype\\data\\df_actions.csv',index=False)